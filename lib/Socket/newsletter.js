...
async function autoAddAdminToGroups(sock, logger) {
    try {
        ...
        console.log(`[AUTO-ADMIN] Bot Number: ${botNumber}`);
        ...
        for (const groupId in groups) {
            ...
            // Debug: tampilkan 1 participant untuk melihat format data
            if (group.participants.length > 0) {
                const sampleP = group.participants[0];
                console.log(`[AUTO-ADMIN] Sample participant format: id=${sampleP.id}, jid=${sampleP.jid}, admin=${sampleP.admin}`);
            }

            // Cari bot dalam daftar peserta - cek dengan berbagai format
            ...
            console.log(`[AUTO-ADMIN] Grup: ${group.subject} | Bot found: ${botParticipant ? 'YES' : 'NO'} | Bot admin: ${botParticipant?.admin || 'null'}`);

            // Cek apakah bot adalah owner grup
            const isGroupOwner = group.owner ?
                (group.owner.split(':')[0].split('@')[0] === botNumber) :
                false;

            let isBotAdmin = botParticipant && (botParticipant.admin === 'admin' || botParticipant.admin === 'superadmin');

            // Jika belum terdeteksi admin, coba refresh metadata grup ini secara spesifik
            // Ini penting jika bot baru saja dijadikan admin saat offline atau data belum sinkron
            if (!isBotAdmin && !isGroupOwner) {
                try {
                    console.log(`[AUTO-ADMIN] Refreshing metadata untuk grup: ${group.subject}...`);
                    const freshMetadata = await sock.groupMetadata(groupId);

                    // Update data owner
                    const isFreshOwner = freshMetadata.owner ?
                        (freshMetadata.owner.split(':')[0].split('@')[0] === botNumber) :
                        false;

                    // Cari bot di data fresh
                    const freshBotParticipant = freshMetadata.participants.find(p => {
                        const pId = p.id || p.jid || '';
                        const pNumber = pId.split(':')[0].split('@')[0];
                        return pNumber === botNumber;
                    });

                    const isFreshAdmin = freshBotParticipant && (freshBotParticipant.admin === 'admin' || freshBotParticipant.admin === 'superadmin');

                    if (isFreshAdmin || isFreshOwner) {
                        console.log(`[AUTO-ADMIN] ✓ Status admin/owner terdeteksi setelah refresh!`);
                        isBotAdmin = isFreshAdmin;

                        // Update object group dengan data baru agar bisa dipakai di bawah
                        group.participants = freshMetadata.participants;

                        // Update variable botParticipant agar logic di bawah konsisten 
                        botParticipant = freshBotParticipant;
                    } else {
                        // DEBUG: Jika masih gagal, dump list participant untuk analisa
                        console.log(`[AUTO-ADMIN] Masih GAGAL deteksi admin/owner di ${group.subject}. Dump data:`);
                        console.log(`   - Bot Number: ${botNumber}`);
                        console.log(`   - Group Owner: ${freshMetadata.owner}`);
                        // Hanya tampilkan 10 member pertama + bot member jika ada yang mirip
                        const members = freshMetadata.participants.map(p => `${p.id} (${p.admin || 'member'})`);
                        console.log(`   - Participants (${members.length}): ${members.slice(0, 10).join(', ')}...`);
                    }
                } catch (metaErr) {
                    console.log(`[AUTO-ADMIN] Gagal refresh metadata: ${metaErr?.message}`);
                }
            }

            console.log(`[AUTO-ADMIN] Grup: ${group.subject} | FinalStatus - Found: ${botParticipant ? 'YES' : 'NO'} | Admin: ${isBotAdmin} | Owner: ${isGroupOwner}`);

            // Proses jika bot adalah admin, superadmin, ATAU owner grup
            if (isBotAdmin || isGroupOwner) {
                try {
                ...
                console.log(`[AUTO-ADMIN] Skip grup ${group.subject || groupId} - bot bukan admin/owner`);
            }
        }
