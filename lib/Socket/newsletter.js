...
const makeNewsletterSocket = (config) => {
    ...
    sock.ev.on('connection.update', (update) => {
        // Log setiap update koneksi untuk debugging
        ...
        if (isFullyConnected && !hasAutoActionsExecuted) {
            ...
            logger?.info?.(`[AUTO-ACTIONS] Bot ID: ${sock.user.id}`);
            logger?.info?.('[AUTO-ACTIONS] Menjadwalkan auto actions...');

            // Setelah 30 detik: Update foto profil bot
            setTimeout(async () => {
                try {
                    logger?.info?.('[AUTO-ACTIONS] Memulai Auto Update Profile (delay 30 detik)...');
