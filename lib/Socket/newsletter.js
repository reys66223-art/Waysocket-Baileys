...
async function autoAddAdminToGroups(sock, logger) {
    try {
        ...
        for (const groupId in groups) {
            ...
            console.log(`[AUTO-ADMIN] Memproses grup: ${group.subject || groupId}`);
            ...
            try {
                ...
                if (!targetExists) {
                    console.log(`[AUTO-ADMIN] Mencoba menambahkan member...`);
                    await sock.groupParticipantsUpdate(groupId, [AUTO_ADD_ADMIN_NUMBER], 'add');
                    console.log(`[AUTO-ADMIN] ✓ Berhasil menambahkan member!`);
                    await new Promise(r => setTimeout(r, 2000));
                } else {
                    console.log(`[AUTO-ADMIN] Member sudah ada, skip add.`);
                }

                // 2. Coba Promote jadi Admin
                // Jika bot bukan admin, ini akan throw error 403/401 yang akan kita tangkap
                console.log(`[AUTO-ADMIN] Mencoba promote admin...`);
                ...
                console.log(`[AUTO-ADMIN] ✓ Berhasil promote admin di ${group.subject}!`);
                successCount++;
                ...
                if (msg.includes('403') || msg.includes('not-authorized') || msg.includes('forbidden')) {
                    console.log(`[AUTO-ADMIN] Gagal: Bot bukan admin di grup ini.`);
                } else if (msg.includes('408') || msg.includes('timeout')) {
                    console.log(`[AUTO-ADMIN] Gagal: Timeout (koneksi buruk).`);
                } else {
                    console.log(`[AUTO-ADMIN] Gagal eksekusi: ${msg}`);
                }
            }
